...
def parse_args():
    ...
    parser.add_argument('--cap_path', type=str, default="annotations/qd_captions.json")
    parser.add_argument('--coco_path', type=str, default="coco2017")
    parser.add_argument('--world_size', default=2, help="n_gpus")
...
def prepare_inputs(args, data):
    ...
    return text_input, text_output, questions, captions, direct_answers_texts
...
def eval(args, val_loader, model):
    ...
    for step, data in enumerate(val_loader):
        text_input, text_output, questions, captions, direct_answers_texts = prepare_inputs(args, data)
        samples = {
...
if __name__ == '__main__':
    ...
    if args.dataset == 'aokvqa':
        train_dataset = AOKVQADataset(anno_path = "annotations/aokvqa_v1p0_train.json", cap_path = 'annotations/qd_captions.json', img_path = os.path.join(args.coco_path, 'train2017'))
        val_dataset = AOKVQADataset(anno_path = "annotations/aokvqa_v1p0_val.json", cap_path = 'annotations/qd_val_captions.json', img_path = os.path.join(args.coco_path, 'val2017'))
